...
static int kate_readbuf(kate_pack_buffer* kpb, char* s, int len) {
  ...
  if ((kate_pack_readable_bits(kpb) + 7) / 8 < len)
    return KATE_E_BAD_PACKET ;
  while (len--)
    *s++ = kate_pack_read(kpb, 8);
  return 0;
}
...
static int kate_decode_check_magic(kate_pack_buffer* kpb) {
  ...
  if (ret < 0)
    return KATE_E_NOT_KATE ;
  if (memcmp(magic, "kate\0\0\0", 7))
    return KATE_E_NOT_KATE ;
...
static int kate_check_eop(kate_pack_buffer* kpb) {
  ...
  if (bits > 0) {
    if (kate_pack_read(kpb, bits))
      return KATE_E_BAD_PACKET ;
  }
  if (kate_pack_look1(kpb) >= 0)
    return KATE_E_BAD_PACKET ; /* no more data expected */
...
static int kate_decode_info_header(kate_info* ki, kate_pack_buffer* kpb) {
  ...
  if (!ki || !kpb)
    return KMG_ERROR(KATE_E_INVALID_PARAMETER);

  ki->bitstream_version_major = kate_pack_read(kpb, 8);
  ki->bitstream_version_minor = kate_pack_read(kpb, 8);
  if (ki->bitstream_version_major > KATE_BITSTREAM_VERSION_MAJOR)
    return KMG_ERROR(KATE_E_VERSION);
  ...
  if (ki->bitstream_version_major == 0 && ki->bitstream_version_minor < 3) {
    if (reserved != 0)
      return KMG_ERROR(KATE_E_BAD_PACKET); /* reserved - 0 */
  }
  ...
  if (ki->granule_shift >= 64)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  if (ki->gps_numerator == 0)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  if (ki->gps_denominator == 0)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  /*
  ...
  */
  ...
  if (!language)
    return KMG_ERROR(KATE_E_OUT_OF_MEMORY);
  ret = kate_readbuf(kpb, language, len); /* a terminating null is included */
  if (ret < 0)
    return KMG_ERROR(ret);
  if (language[len - 1])
    return KMG_ERROR(KATE_E_BAD_PACKET); /* but do check it to be sure */
...
static int kate_decode_comment_packet(const kate_info* ki, kate_comment* kc, kate_pack_buffer* kpb) {
  ...
  if (len < 0)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  if (!ki->no_limits && len > KATE_LIMIT_COMMENT_LENGTH)
    return KMG_ERROR(KATE_E_LIMIT);
...
static int kate_decode_regions_packet(kate_info* ki, kate_pack_buffer* kpb) {
  ...
  if (nregions < 0)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  if (!ki->no_limits && nregions > KATE_LIMIT_REGIONS)
    return KMG_ERROR(KATE_E_LIMIT);
...
static int kate_decode_styles_packet(kate_info* ki, kate_pack_buffer* kpb) {
  ...
  if (nstyles < 0)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  if (!ki->no_limits && nstyles > KATE_LIMIT_STYLES)
    return KMG_ERROR(KATE_E_LIMIT);
...
static int kate_decode_curve(const kate_info* ki, kate_curve* kc, kate_pack_buffer* kpb, kate_memory_guard* parent_kmg) {
  ...
  if (ret < 0)
    return KMG_ERROR(ret);
  if (!ki->no_limits && kc->npts > KATE_LIMIT_CURVE_POINTS)
    return KMG_ERROR(KATE_E_LIMIT);
...
static int kate_decode_curves_packet(kate_info* ki, kate_pack_buffer* kpb) {
  ...
  if (ncurves < 0)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  if (!ki->no_limits && ncurves > KATE_LIMIT_CURVES)
    return KMG_ERROR(KATE_E_LIMIT);
...
static int kate_decode_motion(const kate_info* ki, kate_motion* km, kate_pack_buffer* kpb, kate_memory_guard* parent_kmg) {
  ...
  if (!ki->no_limits && km->ncurves > KATE_LIMIT_MOTION_CURVES)
    return KMG_ERROR(KATE_E_LIMIT);
...
static int kate_decode_motions_packet(kate_info* ki, kate_pack_buffer* kpb) {
  ...
  if (nmotions < 0)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  if (!ki->no_limits && nmotions > KATE_LIMIT_MOTIONS)
    return KMG_ERROR(KATE_E_LIMIT);
...
static int kate_decode_palettes_packet(kate_info* ki, kate_pack_buffer* kpb) {
  ...
  if (npalettes < 0)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  if (!ki->no_limits && npalettes > KATE_LIMIT_PALETTES)
    return KMG_ERROR(KATE_E_LIMIT);
...
static int kate_decode_bitmap(const kate_info* ki, kate_bitmap* kb, kate_pack_buffer* kpb) {
  ...
  if (kb->width <= 0 || kb->height <= 0 || kb->bpp > 8)
    return KATE_E_BAD_PACKET ;
  if (!ki->no_limits && (kb->width > KATE_LIMIT_BITMAP_SIZE || kb->height > KATE_LIMIT_BITMAP_SIZE))
    return KATE_E_LIMIT ;
  ...
  if (kb->bpp == 0) {
    ...
    case kate_bitmap_type_png:
      ...
      if (!ki->no_limits && kb->size > KATE_LIMIT_BITMAP_RAW_SIZE)
        return KATE_E_LIMIT ;
...
static int kate_decode_bitmaps_packet(kate_info* ki, kate_pack_buffer* kpb) {
  ...
  if (nbitmaps < 0)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  if (!ki->no_limits && nbitmaps > KATE_LIMIT_BITMAPS)
    return KMG_ERROR(KATE_E_LIMIT);
...
static int kate_decode_font_ranges_packet(kate_info* ki, kate_pack_buffer* kpb) {
  ...
  if (nfont_ranges < 0)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  if (!ki->no_limits && nfont_ranges > KATE_LIMIT_FONT_RANGES)
    return KMG_ERROR(KATE_E_LIMIT);
  ...
  if (nfont_mappings < 0)
    return KMG_ERROR(KATE_E_BAD_PACKET);
  if (!ki->no_limits && nfont_mappings > KATE_LIMIT_FONT_MAPPINGS)
    return KMG_ERROR(KATE_E_LIMIT);
  ...
  if (nfont_mappings > 0) {
    ...
    for (n = 0; n < nfont_mappings; ++n) {
      ...
      if (nfont_ranges < 0)
        return KMG_ERROR(KATE_E_BAD_PACKET);
      if (!ki->no_limits && nfont_ranges > KATE_LIMIT_FONT_MAPPING_RANGES)
        return KMG_ERROR(KATE_E_LIMIT);
...
int kate_decode_headerin(kate_info* ki, kate_comment* kc, kate_packet* kp) {
  ...
  case 8: /* this is the font ranges list packet */
    ...
    if (ret == 0)
      ret = 1; /* we're done, we know of no more headers to come */
    break;
...
static int kate_decode_text_packet(kate_state* k, kate_pack_buffer* kpb, int repeat) {
  ...
  if (ev->start < 0 || ev->duration < 0)
    goto error_bad_packet;
  if (ev->backlink < 0 || ev->backlink > ev->start)
    goto error_bad_packet;
  ...
  if (len < 0)
    goto error_bad_packet;
  RNDERR(error_bad_packet);
  if (!k->ki->no_limits && len > KATE_LIMIT_TEXT_LENGTH)
    goto error_limit;
  RNDERR(error_limit);

  /* read the text, and null terminate it - 4 characters for UTF-32 */
  if (kate_check_add_overflow(len, 4,NULL))
    goto error_out_of_memory;
  ...
  if (kate_pack_read1(kpb)) {
    ...
    len = kate_read32v(kpb);
    if (len <= 0)
      goto error_bad_packet; /* if the flag was set, there's at least one */
    if (!k->ki->no_limits && len > KATE_LIMIT_TEXT_MOTIONS)
      goto error_limit;
    RNDERR(error_limit);
  ...
  if (((k->ki->bitstream_version_major << 8) | k->ki->bitstream_version_minor) >= 0x0004) {
    ...
    RNDERR(error_bad_packet);
    if (len > 0) {
      if (!k->ki->no_limits && len > KATE_LIMIT_BITMAPS)
        goto error_limit;
  ...
  if (ev->text_markup_type != kate_markup_none && k->ki->remove_markup) {
    ...
    if (ret < 0)
      goto error;
    if (ev->len0 < zero_size)
      goto error_bad_packet; /* can't happen, but let's be safe */
